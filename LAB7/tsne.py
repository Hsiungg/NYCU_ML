...
def x2p(X=np.array([]), tol=1e-5, perplexity=30.0):
    ...
    D = np.add(np.add(-2 * np.dot(X, X.T), sum_X).T, sum_X)     #Dij = ||Xi||^2 - 2*Xi*Xj + ||Xj||^2
    P = np.zeros((n, n))
    beta = np.ones((n, 1))
...
def make_gif(Y, perplexity):
    gif_path = f'./tsne/tsne_{perplexity}.gif'
    out_img = []
...
def plot_similarity(P, Q, perplexity):
    ...
    pylab.tight_layout()
    pylab.savefig(f'./tsne/similarities_tsne_{perplexity}.png')
    pylab.close()
...
if __name__ == "__main__":
    ...
    for perplexity in perplexities:
        ...
        pylab.grid(False)
        pylab.axis('off')
        pylab.savefig(f'./tsne/final_tsne_p={perplexity}.png')
        pylab.close()
